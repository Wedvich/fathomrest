...
function validateDocument(doc: SaveDocument): void {
  ...
  // One warehouse per (island, resource) — the pool invariant enforced at the command boundary
  // (sim.ts addWarehouse). A hand-edited or legacy pre-pool document with two same-typed
  // warehouses on one island would otherwise import cleanly and break the single-pool debit.
  // Nested island -> resources map: tags are arbitrary non-empty strings, so no joined string
  // key is collision-proof.
  const islandResources = new Map<string, Set<string>>();
  const warehouseIds = checkTable(doc.warehouses, "warehouses", (warehouse, at, id) => {
    ...
    const resources = islandResources.get(warehouse.islandId) ?? new Set<string>();
    islandResources.set(warehouse.islandId, resources);
    if (resources.has(warehouse.resource)) {
      throw invalid(
        `${at} duplicate (island, resource): ${warehouse.islandId} already stores ${warehouse.resource}`,
      );
    }
    resources.add(warehouse.resource);
...
// document exactly as it would a native one — note the pool invariant means a v1 document
// with two same-resource warehouses (any routed save) fails that vetting once the backfill
// puts them on one island; those saves land in the app's quarantine path (DESIGN.md's
// blessed pre-pool reset). Only route-free v1 saves still migrate through.
